"""
Memory-efficient sequence alignment algorithm
"""
import sys
import os
import numpy as np
...
# To run:
# python3 efficient_3.py SampleTestCases/input1.txt out/output1.txt

# DNA bases to index
Base = {'A': 0, 'C': 1, 'G': 2, 'T': 3}


def memEffCost(x, y, gap, alpha):
    m, n = len(x), len(y)
    dp = np.empty(n+1)
    dp[0] = 0
    ...
    for i in range(1, m+1):
        ...
        dp[0] = curr
        for j in range(1, n+1):
            curr = min(dp[j]+gap, diag+alpha[Base[x[i - 1]]][Base[y[j - 1]]], curr+gap)
            diag = dp[j]
            dp[j] = curr 
    return dp

def memoryEfficientAlign(x, y, gap, alpha):
    # Base cases
    if len(x)== 0:
        a1, a2 = "", ""
        for i in range(len(y)):
            a1 = a1 + "_"
            a2 = a2 + y[i]
        return gap*len(y), a1, a2 
    elif len(y) == 0:
        a1, a2 = "", ""
        for i in range(len(x)):
            a1 = a1 + x[i]
            a2 = a2 + "_"
        return gap*len(x), a1, a2 
    elif len(x) <= 2 or len(y) <= 2:
        return seqAlign(x, y, gap, alpha)
    else:
        # Divide and conquer
        xmid = len(x) // 2
        
        left = memEffCost(x[:xmid], y, gap, alpha)
        right = memEffCost(x[xmid:][::-1], y[::-1], gap, alpha)
        ymid = np.argmin([left[i]+right[len(y)-i] for i in range(len(y)+1)])
        c1, a1, a2 = memoryEfficientAlign(x[:xmid], y[:ymid], gap, alpha)
        c2, b1, b2 = memoryEfficientAlign(x[xmid:], y[ymid:], gap, alpha) 

    return c1+c2, a1+b1, a2+b2


def runEff(x, y, gap, alpha):
    start_time = time.time()

    cost, str1, str2 = memoryEfficientAlign(x, y, gap, alpha)
    end_time = time.time()
...
def main(argv):
    ...
    cost, align1, align2, time, mem = runEff(final_s1, final_s2, gap, alpha)
    lines = [str(int(cost))+"\n", align1+"\n", align2+"\n", str(time)+"\n", str(mem)+"\n"]
